...
const RLOCK_DEFAULT: CachePadded<AtomicUsize> = CachePadded(AtomicUsize::new(0));
    ...
    T: Sized + Default,
{
    ...
    fn default() -> RwLock<T> {
        RwLock {
            wlock: CachePadded(AtomicBool::new(false)),
    ...
    T: Sized,
{
    ...
    pub fn write(&self, n: usize) -> WriteGuard<T> {
        ...
        loop {
            match self.wlock.0.compare_exchange_weak(
                false,
                true,
                Ordering::Acquire,
                Ordering::Acquire,
        ...
        while !self
            ...
            .iter()
            .take(n)
            .all(|item| item.0.load(Ordering::Relaxed) == 0)
        {
            spin_loop();
        }
    ...
    pub fn read(&self, tid: usize) -> ReadGuard<T> {
        ...
        let ptr = unsafe {
            &*(&self.wlock as *const CachePadded<core::sync::atomic::AtomicBool>
                as *const bool)
        };
        ...
        loop {
            ...
            self.rlock[tid].0.fetch_add(1, Ordering::Acquire);
            if !self.wlock.0.load(Ordering::Relaxed) {
                break;
            }

            self.rlock[tid].0.fetch_sub(1, Ordering::Release);
        }
    ...
    pub unsafe fn read_unlock(&self, tid: usize) {
        if self.rlock[tid].0.fetch_sub(1, Ordering::Release) == 0 {
...
impl<'rwlock, T: Sized> WriteGuard<'rwlock, T> {
