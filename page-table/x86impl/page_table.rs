// From: https://github.com/vmware-labs/node-replicated-kernel/blob/master/kernel/src/arch/x86_64/vspace/page_table.rs
...
use super::paging::*;
use super::{
    kernel_vaddr_to_paddr, paddr_to_kernel_vaddr, Frame, KError, MapAction, TlbFlushHandle,
};
...
impl PageTable {
    ...
    pub fn pml4_address(&self) -> PAddr {
        let pml4_vaddr = VAddr(&*self.pml4 as *const _ as u64);
    ...
    pub(crate) fn map_identity_with_offset(
        ...
        let vbase = VAddr_u64((at_offset + pbase).as_u64());
    ...
    fn can_map_as_huge_page(
        ...
        let vaddr_pos: VAddr = VAddr(PML4_SLOT_SIZE * pml4_idx + HUGE_PAGE_SIZE * pdpt_idx);
        ...
        let no_underlying_2mib_mappings = if !pdpt_entry.is_present() {
            ...
            if all_entries_empty {
                ...
                pdpt[pdpt_idx] = PDPTEntry::new(PAddr::from(0x0), PDPT_empty());
            }
    ...
    fn can_map_as_large_page(
        ...
        // The virtual address corresponding to the current position within the page-table
        let vaddr_pos: VAddr =
            VAddr(PML4_SLOT_SIZE * pml4_idx + HUGE_PAGE_SIZE * pdpt_idx + LARGE_PAGE_SIZE * pd_idx);
        ...
        let no_underlying_4kib_mappings = if !pd_entry.is_present() {
            ...
            if all_entries_empty {
                ...
                pd[pd_idx] = PDEntry::new(PAddr::from(0x0), PD_empty());
            }
            all_entries_empty
        };
    ...
    fn insert_huge_mappings(
        ...
        while mapped < psize && ((psize - mapped) >= HUGE_PAGE_SIZE) && pdpt_idx < pdpt.len() {
            if !insert_mapping {
                ...
                pdpt[pdpt_idx] =
                    PDPTEntry::new(pbase + mapped, PDPT_P | PDPT_PS | rights.to_pdpt_rights());
            }
    ...
    fn insert_large_mappings(
        ...
        while mapped < psize && ((psize - mapped) >= LARGE_PAGE_SIZE) && pd_idx < pd.len() {
            if !insert_mapping {
                ...
                pd[pd_idx] = PDEntry::new(pbase + mapped, PD_P | PD_PS | rights.to_pd_rights());
            }
    ...
    fn insert_base_mappings(
        ...
        while mapped < psize && pt_idx < pt.len() {
            if !insert_mapping {
                ...
                pt[pt_idx] = PTEntry::new(pbase + mapped, PT_P | rights.to_pt_rights());
            }
        ...
        if self.pml4[pml4_idx].is_present() {
            ...
            if pdpt[pdpt_idx].is_present() {
                if pdpt[pdpt_idx].is_page() {
                    ...
                    match action {
                        Modify::Unmap => {
                            pdpt[pdpt_idx] = PDPTEntry::new(PAddr::zero(), 0x0);
                        }
                        Modify::UpdateRights(new_rights) => {
                            let flags = PDPT_P | PDPT_PS | new_rights.to_pdpt_rights();
                    ...
                    if pd[pd_idx].is_present() {
                        if pd[pd_idx].is_page() {
                            ...
                            match action {
                                Modify::Unmap => {
                                    pd[pd_idx] = PDEntry::new(PAddr::zero(), 0x0);
                                }
                                Modify::UpdateRights(new_rights) => {
                                    let flags = PD_P | PD_PS | new_rights.to_pd_rights();
                            ...
                            if pt[pt_idx].is_present() {
                                ...
                                match action {
                                    Modify::Unmap => {
                                        pt[pt_idx] = PTEntry::new(PAddr::zero(), 0x0);
                                    }
                                    Modify::UpdateRights(new_rights) => {
                                        let flags = PT_P | new_rights.to_pt_rights();
    ...
    fn alloc_frame(&self) -> Frame {
        ...
        let vaddr = VAddr(frame_ptr.as_ptr() as *const u8 as u64);
    ...
    fn new_pt(&self) -> PDEntry {
        ...
        return PDEntry::new(frame.base, PD_P | PD_RW | PD_US);
    }

    fn new_pd(&self) -> PDPTEntry {
        let frame = self.alloc_frame();
        return PDPTEntry::new(frame.base, PDPT_P | PDPT_RW | PDPT_US);
    }

    fn new_pdpt(&self) -> PML4Entry {
        let frame = self.alloc_frame();
        return PML4Entry::new(frame.base, PML4_P | PML4_RW | PML4_US);
    }
