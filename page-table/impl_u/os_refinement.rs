...
pub proof fn lemma_effective_mappings_equal_interp_pt_mem(this: OSVariables)
    requires
        //this.tlb_is_submap_of_pt()
    ensures
        this.effective_mappings().submap_of(this.interp_pt_mem()),
{
    assume(false);
...
proof fn init_refines_hl_init(c: os::OSConstants, s: os::OSVariables)
    requires
        os::init(c, s)
    ensures
        hlspec::init(c.interp(s), s.interp(c))
{
    let abs_c = c.interp(s);
...
// also needs invariant that tlbs are submap of pt 
proof fn next_step_refines_hl_next_step(c: os::OSConstants, s1: os::OSVariables, s2: os::OSVariables, step: os::OSStep)
    ...
    ensures
        hlspec::next_step(c.interp(s1), s1.interp(c), s2.interp(c), step.interp())
{
    match step {
