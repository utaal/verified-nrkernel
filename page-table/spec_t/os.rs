...
impl OSVariables {
	...
	*/
    ...
    pub open spec fn is_effective_mappings(self, vmem_idx: nat, pte: PageTableEntry) -> bool {
        exists |core: Core| #[trigger] self.hw.NUMAs.contains_key(core.NUMA_id) 
            && #[trigger] self.hw.NUMAs[core.NUMA_id].cores.contains_key(core.core_id) 
            && #[trigger] self.hw.NUMAs[core.NUMA_id].cores[core.core_id].tlb.contains_pair(vmem_idx, pte)

    }

    pub open spec fn joint_tlbs(self) -> Map<nat, PageTableEntry> {
        Map::new( |vmem_idx: nat| { exists |core: Core| self.hw.NUMAs.contains_key(core.NUMA_id) 
            && self.hw.NUMAs[core.NUMA_id].cores.contains_key(core.core_id) 
            && self.hw.NUMAs[core.NUMA_id].cores[core.core_id].tlb.contains_key(vmem_idx)},
            |vmem_idx: nat| {choose |pte: PageTableEntry| self.is_effective_mappings( vmem_idx, pte)},
