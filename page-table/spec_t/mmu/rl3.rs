    ...
    match step {
        Step::Invlpg => {
            ...
            if core == pre.writes.core {
                assert(forall|core| c.valid_core(core) ==> post.mem_view_of_core(core) == post.mem_view_of_writer());
                assert(post.inv_x(c));
            } else {
