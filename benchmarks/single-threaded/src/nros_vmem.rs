// Code adapted from:
// https://github.com/vmware-labs/node-replicated-kernel/blob/a6b4c35fd35bf80a634702b91ec0cd4b27cfb341/kernel/src/arch/x86_64/vspace/page_table.rs
// https://github.com/vmware/node-replication/blob/f005642a832abbc8e05f75857af0dd0c3f0a07ce/node-replication/benches/vspace.rs
...
impl Into<MapAction> for PDPTFlags {
    fn into(self) -> MapAction {
        MapAction::None
    }
}

impl Into<MapAction> for PTFlags {
    fn into(self) -> MapAction {
        MapAction::None
    }
}

impl Into<MapAction> for PDFlags {
    fn into(self) -> MapAction {
        MapAction::None
    }
}
...
impl VSpace {
    pub fn unmap(&mut self, base: VAddr) -> Result<(), ()> {
    ...
    pub fn map_generic(
        ...
        if !self.pml4[pml4_idx].is_present() {
            // println!("New PDPDT for {:?} @ PML4[{}]", vbase, pml4_idx);
        ...
        if !pd[pd_idx].is_present() {
                ...
                && psize >= LARGE_PAGE_SIZE
            {
                ...
                while mapped < psize && ((psize - mapped) >= LARGE_PAGE_SIZE) && pd_idx < 512 {
                    if pd[pd_idx].is_present() {
                        // println!("Already mapped pd at {:#x}", pbase + mapped);
                        return Err(VSpaceError { at: vbase.as_u64() });
                    }
    ...
    fn modify_generic<'a>(
        ...
        addr: VAddr,
        action: Modify,
    ) -> Result<(VAddr, PAddr, usize, MapAction), ()> {
        let pml4_idx = pml4_index(addr);
        if self.pml4[pml4_idx].is_present() {
            let pdpt_idx = pdpt_index(addr);
